...
Entries:
  ...
  todo entry remind ID --in=5m|--at="20:00"|--clear
USAGE
}
...
ensure_db() {
  ...
  "
  # Migration: add reminder columns (ignore if already exist)
  sql "ALTER TABLE entries ADD COLUMN reminder_at TEXT DEFAULT NULL;" 2>/dev/null || true
  sql "ALTER TABLE entries ADD COLUMN cron_job_id TEXT DEFAULT NULL;" 2>/dev/null || true
}
...
cancel_reminder_silent() {
  local id="$1"
  local job_id
  job_id="$(sql_scalar "SELECT cron_job_id FROM entries WHERE id=$id;")"
  if [[ -n "$job_id" ]]; then
    openclaw cron rm "$job_id" --json >/dev/null 2>&1 || true
    sql "UPDATE entries SET reminder_at=NULL, cron_job_id=NULL, updated_at=CURRENT_TIMESTAMP WHERE id=$id;"
  fi
}

cancel_group_reminders_silent() {
  local gid="$1"
  local ids
  ids="$(sql_scalar "SELECT id FROM entries WHERE group_id=$gid AND cron_job_id IS NOT NULL;")"
  local eid
  while IFS= read -r eid; do
    if [[ -n "$eid" ]]; then cancel_reminder_silent "$eid"; fi
  done <<< "$ids"
}
  ...
  case "$action" in
    ...
    remove)
      ...
      if [[ "$(flag_get delete-entries false)" == "true" ]]; then
        cancel_group_reminders_silent "$gid"
...
cmd_entry() {
  ...
  case "$action" in
    ...
    list)
      ...
      local where="1=1"
      local select="SELECT e.id, e.status, g.name, CASE WHEN e.reminder_at IS NOT NULL AND e.reminder_at > datetime('now') THEN '⏰ ' ELSE '' END || e.text FROM entries e JOIN groups g ON g.id=e.group_id"
    ...
    show)
      ...
      sqlite3 -batch -line "$DB" "
        SELECT e.id AS id, e.status AS status, g.name AS group_name, e.text AS text,
               e.reminder_at AS reminder_at, e.created_at AS created_at, e.updated_at AS updated_at
    ...
    remove)
      ...
      cancel_reminder_silent "$id"
      sql "DELETE FROM entries WHERE id=$id;"
      echo "ok: entry removed: $id"
      ;;

    remind)
      [[ ${#REST[@]} -eq 1 ]] || die "entry remind requires exactly 1 arg: ID"
      require_int "${REST[0]}"
      local id="${REST[0]}"
      ...
      local clear; clear="$(flag_get clear "false")"
      if [[ "$clear" == "true" ]]; then
        cancel_reminder_silent "$id"
        echo "ok: reminder cleared: $id"
        return
      fi

      local in_val; in_val="$(flag_get in "")"
      local at_val; at_val="$(flag_get at "")"

      [[ -n "$in_val" || -n "$at_val" ]] || die "entry remind requires --in=DURATION or --at=TIME or --clear"
      [[ -z "$in_val" || -z "$at_val" ]] || die "use --in or --at, not both"

      # Cancel existing reminder first
      cancel_reminder_silent "$id"

      local at_arg reminder_display
      if [[ -n "$in_val" ]]; then
        # Relative: pass duration directly to openclaw cron --at
        [[ "$in_val" =~ ^[0-9]+[smhd]$ ]] || die "invalid --in format: use e.g. 5m, 1h, 2d"
        at_arg="$in_val"
        reminder_display="in $in_val"
      else
        # Absolute: parse as Beijing time, convert to UTC ISO
        if [[ "$at_val" =~ ^[0-9]{1,2}:[0-9]{2}$ ]]; then
          # Time only — today in BJT, roll to tomorrow if past
          local target_epoch now_epoch
          now_epoch=$(TZ=Asia/Shanghai date +%s)
          target_epoch=$(TZ=Asia/Shanghai date -d "today $at_val" +%s 2>/dev/null) || die "invalid time: $at_val"
          if (( target_epoch <= now_epoch )); then
            target_epoch=$(TZ=Asia/Shanghai date -d "tomorrow $at_val" +%s 2>/dev/null) || die "invalid time: $at_val"
          fi
          at_arg=$(date -u -d "@$target_epoch" +%Y-%m-%dT%H:%M:%S.000Z)
          reminder_display="at $at_val BJT"
        elif [[ "$at_val" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2} ]]; then
          # Full datetime in BJT
          local dt_epoch
          dt_epoch=$(TZ=Asia/Shanghai date -d "$at_val" +%s 2>/dev/null) || die "invalid datetime: $at_val"
          at_arg=$(date -u -d "@$dt_epoch" +%Y-%m-%dT%H:%M:%S.000Z)
          reminder_display="at $at_val BJT"
        else
          die "invalid --at format: use HH:MM or YYYY-MM-DD HH:MM"
        fi
      fi

      # Get entry text
      local entry_text
      entry_text="$(sql_scalar "SELECT text FROM entries WHERE id=$id;")"
      ...
      cron_output=$(openclaw cron add \
        --name "todo-remind-${id}" \
        --at "${at_arg}" \
        --message "⏰ 待办提醒: ${entry_text} (ID: ${id})" \
        --announce \
        --channel last \
        --delete-after-run \
        --json 2>&1) || die "failed to schedule reminder: $cron_output"

      local job_id
      job_id=$(echo "$cron_output" | jq -r '.id') || die "failed to parse cron job id"
      [[ -n "$job_id" && "$job_id" != "null" ]] || die "cron job id is empty"

      # Store reminder metadata
      local reminder_at
      reminder_at=$(echo "$cron_output" | jq -r '.schedule.at') || reminder_at=""

      sql "UPDATE entries SET reminder_at=$(sql_q "$reminder_at"), cron_job_id=$(sql_q "$job_id"), updated_at=CURRENT_TIMESTAMP WHERE id=$id;"
      echo "ok: reminder set: $id ($reminder_display)"
      ;;
